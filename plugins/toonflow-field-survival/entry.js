...
function tickSurvival(state) {
    ...
    if (state.hunger === 0 || state.thirst === 0) {
        ...
        if (state.hunger === 0)
            state.events.push("饥饿难耐，身体虚弱...");
        if (state.thirst === 0)
            state.events.push("口渴难忍，意识模糊...");
    }
    // 全部满时小幅度恢复
...
// ---------------------------------------------------------------------------
export async function handle_action(action, params, state, _context) {
    // 首次调用或无状态时初始化
    ...
    switch (action) {
        ...
        case "find_food": {
            tickSurvival(s);
            if (!s.alive)
                return { code: 1, message: "已死亡", state: s };
        ...
        case "attack": {
            ...
            const enemy = target
                ? aliveEnemies.find(e => e.id === target || e.name.includes(target)) || aliveEnemies[0]
                : aliveEnemies[0];
